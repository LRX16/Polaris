...
export default function InterviewRoom({ settings, questions, onFinishSession, onGoBack }: InterviewRoomProps) {
  ...
  const handleEvaluateAnswer = async () => {
    ...
    try {
      ...
      const roleStr = settings.role || "Professional";
      ...
      const dynamicStarFeedback = {
        situation: `Presented clear context of a real-world scenario encountering high requirements as a ${lvlStr} ${roleStr} at ${compStr}.`,
        task: `Identified concrete targets, resource limits, and architectural or process demands associated with ${roleStr} expectations.`,
        action: `Detailed logical step-by-step resolution steps, leveraging collaboration and strategic communication matching a ${lvlStr} level.`,
        result: `Quantified successful performance indices and team workflows, confirming positive feedback from managers.`
      };

      // Generous, highly contextual resilience fallback
      const fallbackScores: AnswerScores = {
        ...
        engagementScore: Math.round((simMovement + calculatedPace) / 2),
        professionalismScore: Math.round((simPosture + simHandControl) / 2),
        nervousnessScore: Math.max(0, 100 - Math.round((simHandControl + calculatedConfidence + calculatedPause) / 3)),
        communicationScore: Math.round((calculatedPace + 90 + calculatedPause) / 3),
        starFeedback: dynamicStarFeedback,
        fillerWordsFound: ["like"],
        tips: [
          `Incorporate direct metrics for ${roleStr} activities`,
          `Decrease pauses and keep posture alignment high`,
          `Elevate executive summary vocabulary tailored to ${lvlStr} expectations`
        ]
      };
